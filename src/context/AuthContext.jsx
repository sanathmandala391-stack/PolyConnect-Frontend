...
export function AuthProvider({ children }) {
  ...
  const refreshMe = useCallback(async () => {
    ...
    try {
      ...
      // Only clear session if token is explicitly rejected by the server (401 / 403)
      if (err?.response?.status === 401 || err?.response?.status === 403) {
        localStorage.removeItem("pc_token");
        localStorage.removeItem("pc_user");
        setUser(null);
      }
      // If it's a network glitch, offline mode, or temporary server restart, keep the cached user session intact!
    } finally {
      setLoading(false);
    }
  }, []);
